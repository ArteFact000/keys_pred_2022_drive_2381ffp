...
def start_a_competition(conn, name=None, date=None, organizer=None, location=None):
    ...
    sql = "CREATE TABLE IF NOT EXISTS competition_{} (id INTEGER PRIMARY KEY AUTOINCREMENT, ride_type text, pilots_involved text, begin_time text, end_time text, name text);".format(get_competition_id(conn))
    print(sql)
    c.execute(sql)
    conn.commit()
...
def start_a_ride(conn, competition_index='1', ride_type='64', pilots=[], begin_time='00:00', end_time='00:30'):
    ...
    c = conn.cursor()
    c.execute(sql_create_ride)
    sqlt = 'CREATE TABLE IF NOT EXISTS "{}.{}" (id INTEGER PRIMARY KEY AUTOINCREMENT, pilot text, angle text, velocity text, trajectory text);'.format(competition_index, get_ride_id(conn, competition_index))
    print(sqlt)
    c.execute(sqlt)
    conn.commit()
